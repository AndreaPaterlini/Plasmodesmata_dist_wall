...
In this section we continue to ask questions about the distribution of Plasmodesmata (or similar types of annotations along a given model). We continue to use the output of the *SpatialControlPoints* plugin shown in the [Distributions section](#distributions.html). Since we detected a bias in the distribution of Plasmodesmata, strongly hinting at the presence of spatial clustering, we now ask: **Can we quantify parameters relating to these clusters, such as their number and their areas?**
...
Col_real <- list.files(path = ".", pattern = "Ann.csv") %>% 
  map_df(function(f){
    ...
    x <- read_csv(f) %>% 
      ...
      setnames(old = c('LabelPositionXunits','LabelPositionYunits', 'LabelPositionZunits'), new = c('X_units','Y_units', 'Z_units')) %>%
...
# we are only going to use the coordinates of the real points in this case 
# so we will start from the Col_real object

# we need to look at each cell individually before we run the function described later on
# so we need to filter the original file 
# it is actually easier to split the larger file into smaller datasets corresponding to different roots for example.
...
library(stringr)
Col_real$DatasetFilename <- sapply(str_split(Col_real$DatasetFilename, ".PPP"), head, 1)
...
# this can be automated in the function described below and does not require inspection of the data
  ...
  # the method requires the following library
  library(mclust) 
...
clusters_count %>%
  ggplot(aes(x= Method, y=n, colour= Genotype, fill= Genotype)) + theme_bw() +
  geom_violin(alpha=0.5) + 
  stat_summary(fun.y = median, fun.ymin = median, fun.ymax = median,
               geom = "crossbar", size = 0.5, width = 0.3, alpha=1)  + 
  geom_quasirandom(data= clusters_count, aes(group=Genotype), colour="black", fill="black", size= 2, alpha=0.5, width=0.1, show.legend = FALSE, dodge.width = 0.9) +
  ...
  scale_color_manual(values=c("#8B8B83", "#008080")) +
  scale_fill_manual(values=c("#8B8B83", "#008080")) +
...
combined_count %>%
  ...
  geom_quasirandom(data= combined_count, aes(group=Genotype), colour="black", fill="black", size= 2, alpha=0.5, width=0.1, show.legend = FALSE, dodge.width = 0.9) +
  stat_summary(fun.y = median, fun.ymin = median, fun.ymax = median,
               geom = "crossbar", size = 0.5, width = 0.3, alpha=1)  + 
