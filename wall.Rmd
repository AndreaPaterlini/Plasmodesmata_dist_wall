...
Col_allwall <- Col_allwall %>% setnames(old = c('Matrix.of.cell.wall.thickness.values.in.micrometers.for.each.point.of.the.masked.centreline'), new = c('Thickness_um'))
...
Col_wall_pd <- select(Col_wall, "Genotype", "Interface", "DatasetFilename", "WallThicknessRealPores_um") %>% 
  ...
  setnames(old = c('WallThicknessRealPores_um'), new = c('Thickness_um')) %>%
...
Col_wall_sim <- select(Col_wall, "Genotype", "Interface", "DatasetFilename", "WallThicknessRandomPores_um") %>% 
  setnames(old = c('WallThicknessRandomPores_um'), new = c('Thickness_um')) %>%
...
#we are splitting the column just before "PPP" therefore taking _ as well 
# and retains first part of the split (it takes some time)
library(stringr)
Col_0$DatasetFilename <- sapply(str_split(Col_0$DatasetFilename, ".PPP"), head, 1)
...
ggplot(Col_0, aes(x = Positions, y = Thickness_nm, fill=Positions, colour=Positions)) + theme_bw() +
  ...
  geom_quasirandom(data=stats, aes(x = Positions, y = mean_thickness, shape=DatasetFilename), width = 0.2, size= 2, colour="black", alpha=0.5) +
