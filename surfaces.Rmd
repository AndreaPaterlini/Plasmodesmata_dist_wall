...
# Calculating the median total conductive surface of an interface (as %) {#surf1}
...
# Calculating the median surface of individual clusters (as %) {#surf2}
...
For "XY smoothing", "XY sampling" and "Z sampling" we imput values of *5, 5 and 1*. This creates an appropriate level of surface smoothing for the surface we are trying to generate and ensures a good yet computationally feasible sampling of our models to generate the surface itself.
Clicking "Save results in" enables you to store the surface area values and by pressing "Filename for export" you can ensure that the format is *.csv*. "Export contact To Imaris" enables to send the surface to Imaris for 3D rendering.
...
This is the output of the *ContactArea3D* plugin: a <i>\*_ContactArea3D_wall.csv</i> file. 
...
Col_area$DatasetFilename <- gsub("_ContactArea3D_wall.csv", "", Col_area$DatasetFilename)
...
Col_area_corrected %>%
  ggplot(aes(x= Genotype, y=total_surface, colour= Genotype, fill= Genotype)) + theme_bw() +
  geom_violin(alpha=0.5) + 
  stat_summary(fun.y = median, fun.ymin = median, fun.ymax = median,
               geom = "crossbar", size = 0.5, width = 0.3, alpha=1)  + 
  geom_quasirandom(colour="black", fill="black", size= 2, alpha=0.5, width=0.1, show.legend = FALSE, dodge.width = 0.5) +
